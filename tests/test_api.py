...
@pytest.mark.asyncio
async def test_act_and_state(client):
...
@pytest.mark.asyncio
async def test_policy_reload(client):
    r = await client.post("/policy/reload", headers=AUTH_HEADER)
    assert r.status_code == 200
    assert "rule_count" in r.json() or "loaded_at" in r.json()

@pytest.mark.asyncio
async def test_plan_whitelist_denied(client):
    r = await client.post("/plan", headers=AUTH_HEADER, json={"intent": "ok plan", "context": "demo", "target_paths": ["/etc/passwd"]})
    assert r.status_code in (400,403)

@pytest.mark.asyncio
async def test_plan_risk_gate_denied(client):
...
@pytest.mark.asyncio
async def test_plan_created(client):
...
@pytest.mark.asyncio
async def test_prompt_injection_smoke(client):
...
@pytest.mark.asyncio
async def test_sse_heartbeat(client):
